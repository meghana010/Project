...
const {UserModel,userTable2,userTable,userTable3,userTable4,userTable5,userTable6,UserModel2} = require("./models/user");
...
const y=  foo(req.body); */
 UserModel.findOne({FullName:req.body.FullName,email:req.body.email,contactNo:req.body.contactNo,Occupation:req.body.Occupation,Amount:req.body.Amount,adopt:req.body.adopt,address:req.body.address,DateOfRegistration:req.body.DateOfRegistration},
function(err,result){
  if(err)
  throw err;
  if(!result)
  {
    ...
    userTable2.findOneAndUpdate({ childId: id},update, 
      {
    ...
    userTable.findOneAndUpdate({ childId: id},update, 
      {
          returnNewDocument: true
      } ,function( error, result){
      if(error)
    ...
    userTable3.findOneAndUpdate({ childId: id},update, 
      {
          returnNewDocument: true
      } ,function( error, result){
      if(error)
    ...
    userTable4.findOneAndUpdate({ childId: id},update, 
      {
          returnNewDocument: true
      } ,function( error, result){
      if(error)
    ...
    userTable5.findOneAndUpdate({ childId: id},update, 
      {
          returnNewDocument: true
      } ,function( error, result){
      if(error)
    ...
    userTable6.findOneAndUpdate({ childId: id},update, 
      {
          returnNewDocument: true
      } ,function( error, result){
      if(error)
